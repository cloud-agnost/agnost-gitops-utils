...
export const connectToRedisCache = async () => {
	try {
		...
		client.on("connect", function () {
			...
			console.info(
				`Connected to the cache server @${process.env.CACHE_HOSTNAME}:${process.env.CACHE_PORT}`
			);
		});
	...
	if (
		process.env.CACHE_READ_REPLICA_HOSTNAME &&
		process.env.CACHE_READ_REPLICA_PORT
	) {
		try {
			clientReadReplica = redis.createClient({
				host: process.env.CACHE_READ_REPLICA_HOSTNAME,
				port: process.env.CACHE_READ_REPLICA_PORT,
				password:
			...
			clientReadReplica.on("connect", async function () {
				...
				console.info(
					`Connected to the read replica cache server @${process.env.CACHE_READ_REPLICA_HOSTNAME}:${process.env.CACHE_READ_REPLICA_PORT}`
				);
			});

			clientReadReplica.on("error", function (err) {
				console.error(`Cannot connect to the replica cache server`, {
					details: err,
				});
				process.exit(1);
			});
		} catch (err) {
			console.error(`Cannot connect to the cache read replica server`, {
				details: err,
			});
			process.exit(1);
		}
